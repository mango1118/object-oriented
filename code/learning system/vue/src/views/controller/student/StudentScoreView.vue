...
export default {
  ...
  methods: {
    ...
    async sendReq() {
      // this.pageNum = 1;
      const resp = await this.axios.get("/studentPapers/pageLikeScore?pageNum="+this.pageNum+"&pageSize="+this.pageSize+"studentId="+this.studentNow.id);
      // console.log(resp);
    ...
    async sendLikeReq() {
      // this.pageNum = 1;
      const resp = await this.axios.get(`/studentPapers/pageLikeScore?pageNum=${this.pageNum}&pageSize=${this.pageSize}&studentPaperId=${this.studentPaperId}&studentPaperName=${this.studentPaperName}&studentId=${this.studentNow.id}`);
      // console.log(resp);
      this.tableData = resp.data.data;
      this.total = resp.data.total;
    },
