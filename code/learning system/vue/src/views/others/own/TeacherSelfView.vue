    ...
    <div style="box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04); width: 750px">
      ...
      <el-form :model="saveform" label-width="80px" size="small" style="width: 600px">
        ...
        <el-form-item label="密码" prop="password">
          <el-input v-model="saveform.teacherPassword" autocomplete="off" show-password></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    sendReq() {
      this.axios.get(`/teachers/${this.teacherNow.id}`).then(res => {
        console.log(res)
        localStorage.setItem("user", JSON.stringify(res.data));
        this.saveform = res.data
        this.teacherNow = res.data
      })
    },
    saveForm() {
      this.saveform.level = this.teacherNow.level
      this.axios.put("/teachers", this.saveform).then(res => {
    ...
    resetForm() {
      this.sendReq()
      this.saveForm = this.teacherNow
    },
  },
  mounted: function () {
    // this.sendLikeReq();
    this.teacherNow = JSON.parse(localStorage.getItem("user"))
    console.log(this.teacherNow)
    this.saveform = this.teacherNow
  }
}
</script>
